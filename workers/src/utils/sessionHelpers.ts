...
export function createSessionCookie(sessionId: string): string {
  // Use SameSite=None with Secure flag for cross-site cookies in production
  // This allows cookies to work between Pages (frontend) and Workers (backend)
  return `${SESSION_COOKIE_NAME}=${sessionId}; HttpOnly; SameSite=None; Secure; Path=/; Max-Age=${SESSION_MAX_AGE}`;
}
